...
from config.hex_setting import TARGET_SOC, TARGET_SOC_STD, HIGH_SOC_THRESHOLD, INIT_SOC_STD,LOW_SOC_THRESHOLD
import numpy as np
...
class VehicleState(object):
    ...
    __slots__ = [
        ...
        'travel_dist', 'price_per_travel_m', 'price_per_wait_min', 'assigned_mid',  'charge_flag', 'attack_status',
    ...
    def __init__(self, id, location, hex_id, current_SOC, agent_type,attack_status):
        self.id = id
        self.vehicle_id = id
        ...
        self.charge_flag = 0
        self.attack_status = attack_status # Susceptible - Infectious.
        ...
        self.target_SOC = round(min(1, max(0, np.random.normal(TARGET_SOC, TARGET_SOC_STD))),3)
        self.set_price_rates()
        self.SOC = current_SOC # round(min(1, max(LOW_SOC_THRESHOLD, np.random.normal(HIGH_SOC_THRESHOLD, INIT_SOC_STD))),3)  # SOC~N(90%,2%)
        self.charging_threshold = LOW_SOC_THRESHOLD
