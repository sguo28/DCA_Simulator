...
class Vehicle(object):
    ...
    def __init__(self, vehicle_state,with_option,local_matching,with_charging,tick,reset_every_day=False):
    ...
    def reset_stored_od_pairs(self):
        self.matching_od_pairs = []
        self.charging_od_pairs = []
        self.repositioning_od_pairs = []
    ...
    def send_to_dispatching_pool(self, action_select,action_execute,opts=-1,ctd_opt=0):
        '''
        assigned_action: selected by DQN
    ...
    def softmax_sending_to_charging_station_pool(self):
        self.__change_to_tobedispatched()


    def send_to_charging_station_pool(self,action_select,action_execute,opts = -1, ctd_opt=0):
        '''
        :param action_select: action for DQN training
        :param action_execute: action that is actually implemented
        :param opts: selected option, -1 by default indicates no option.
        '''
        self.state.dispatch_action_id = action_execute
        self.action = action_execute
        self.assigned_action = action_select
    ...
    def location_interp(self, t_unit=60):
        ...
        if isinstance(self.state.time_to_destination, list):
            if self.state.time_to_destination[0]>0: #valid trajectories
                ...
                #     time_ticks.append(total_tt)  # add the final_trb step
            ...
            # empty routes (not feasible)
                ...
                if self.state.status<=1 or self.state.status==9:  # 9 is the stay
                    K = 1 #IDLE_DURATION, 1 min
                else:
                    K= 1 #no wait , for charging, dispatching, assigned status, 1 min
    ...
    def park(self, tick, hex_zones):
        '''
        The status park is triggered when Cruising is finished, keep Staying, initiate an OffDuty vehicle.
        '''
        #update the hex zones
        hex_zones[self.state.hex_id].remove_veh(self) # vehicle has left
        hex_zones[self.cruise_dest].add_veh(self) # vehicle is about to arrive
        self.state.hex_id = self.cruise_dest # update the location.
        self.start_serve_tick = tick  # start serve tick is for full service cycle if being matched next

        if (tick+60)//60%1440==0 and self.reset_every_day: # the end of one day
            terminate=True
            self.end_tick=tick
        else:
            terminate=False
        ...
        self.repositioning_od_pairs.append([self.opt_start_state[0], self.opt_start_state[1], self.cruise_dest,
                                            self.dist_to_destination,self.time_to_destination,self.assigned_action])
        # relocation start time, relocation from, relocation to, travel dist, travel time, use option or not and which option (if used)
        ...
        if len(self.cruise_trajectory)>9: # todo:why?
            self.trajectory_transition.append(self.cruise_trajectory)
            self.cruise_trajectory=[]

        if self.state.SOC < 0: # battery depleted
        ...
        if self.assigned_option==-1: #no assigned policy
            ...
            # if self.state.id == status_codes.V_CRUISING:
            ...
            if self.on_random:  #following options
                self.f_transition.append([self.opt_start_state, self.f_end_state,1])
    ...
    def heading_to_charging_station(self, cs_id, cs_coord, tick, action_id):
        assert self.__behavior.available
    ...
    def heading_to_customer(self, destination, customer_hex_location,tick):
        '''
        :destination: lon, lat
        '''
        assert self.__behavior.available
    ...
    def pickup(self,tick):
        ...
        self.f_end_state = [self.end_tick, self.state.hex_id, self.state.SOC]  # if not cruising ,meaning idle or stay, start a new transition for f
        # # #
        # self.f_transition.append([self.f_start_state, self.f_end_state,0])
        # if self.state.id==1:
    ...
    def dropoff(self, tick,hex_zones):
        ...
        # customer_payment=5
        # todo: disable it when running continuous days.
        # if (tick + 60) // 60 % 1440 == 0:
        #     terminate = True
        #     self.end_tick=tick
        # else:
        ...
        if self.assigned_option==-1: # without option.
            temp_reward = self.reward - self.opt_reward
            steps = max((self.end_tick - self.opt_start_tick) // 60,1)
            rate = (self.discount_factor ** steps - 1) / (steps * (self.discount_factor - 1)) #discounted rate
            ...
            self.recent_transitions.append([self.opt_start_state, action, self.end_state, temp_reward*rate, terminate,
                                            (self.end_tick - self.opt_start_tick) // 60 ]) # sometime it is negative.
            # if self.state.id==status_codes.V_CRUISING:
        ...
        self.state.assigned_customer_id = None
        self.__customers_ids = []
        self.__change_to_idle()
    ...
    def start_charge(self):
        # print(f"\n====={self.state.id}, start charging with SOC={self.state.SOC}\n")
    ...
    def end_charge(self, tick, unit_time_price):
        ...
        # print(f"\n====={self.state.id}, finished charging with SOC={self.state.SOC}\n")
        # self.__change_to_charging()
    ...
    def DoS_end_charge(self, tick):
        ...
        cumulative_discounted_reward = self.decay_lambda * np.sum([self.discount_factor**(t+1) for t in range((self.end_tick-self.start_wait_tick)//60)])  # 60 is t_unit
        self.decay_lambda *= self.discount_factor**((self.end_tick-self.start_wait_tick)//60)  # update lambda, the last update was when it arrived at charging station
        self.reward += - BETA_TIME * 60*cumulative_discounted_reward
        # terminate_flag=False
    ...
    def update_time_to_destination(self):
        ...
        if len(self.time_ticks) > 0:
            ...
            if self.state.status not in [status_codes.V_OCCUPIED, status_codes.V_ASSIGNED]:
    ...
    def add_charging_wait_time(self, wait_time):
        self.charging_wait += wait_time
